...
/*   Updated: 2022/02/10 16:11:50 by rafernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	if (argc != 5 && argc != 6)
	{
		ft_putstr(STDERR_FILENO,
			"Usage: ./prog number_of_philosophers time_to_die time_to_eat \
time_to_sleep [number_of_times_each_must_eat]\n");
		return (-1);
	}
	if (ph_parse_args(argc, argv, &args) == -1)
		return (1);
	ph_init_philosophers(&args);
...
static int	ph_parse_args(int argc, char **argv, t_args *args)
{
	...
	if (argc == 6)
		(args->eat_ammount) = ft_atoi(argv[5]);
	else
		(args->eat_ammount) = 1;
